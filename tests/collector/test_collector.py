...
def test_get_users_metrics_df(dfs, coll_w_users_df):
    users_df, sessions_df, = dfs
    result_df = coll_w_users_df.get_user_metrics_df(sessions_df)
    assert len(result_df) > 10


def test_get_users_metrics_df_counts(old_result, dfs, coll_w_users_df):
    users_df, sessions_df, = dfs
